...
- [ ] README 프로젝트 개요 작성
- [ ] README 수행 항목 체크리스트 작성
- [ ] 각 섹션별 검증 방법(어떤 명령으로 무엇을 확인했는지) + 결과 증거 위치(스크린샷 파일명, 코드블록 등) 링크 정리
- [ ] 트러블슈팅 2건 이상 작성 (문제 → 원인 가설 → 확인 → 해결/대안)
- [ ] 모든 스크린샷/로그에서 민감정보 마스킹 확인
- [ ] README만으로 전체 수행 과정을 파악할 수 있는지 셀프 검토
- [ ] 최종 커밋 및 푸시
- [ ] GitHub 저장소 공개 설정 확인
